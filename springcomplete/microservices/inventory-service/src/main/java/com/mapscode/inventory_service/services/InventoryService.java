package com.mapscode.inventory_service.services;

import com.mapscode.inventory_service.model.entities.Inventory;
import com.mapscode.inventory_service.repositories.InventoryRepository;
...
public class InventoryService {
    ...
    public BaseResponse areInStock(List<OrderItemRequest> orderItems) {
        // Record the whole possible errors in a list
        var errorList = new ArrayList<String>();
        // Become the result in a list of strings with OrderItemRequest::getSku
        List<String> skus = orderItems.stream().map(OrderItemRequest::getSku).toList();
        List<Inventory> inventoryList = inventoryRepository.findBySkuIn(skus);

        // It wiil return the whole corresponding records that are present on the table
